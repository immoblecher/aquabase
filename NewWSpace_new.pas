...
begin
  ...
  begin
    ...
    MessageDlg('This folder is an Aquabase workspace already! Please choose a different folder.',
      mtWarning, [mbOK], 0);
  end
  else
  if not FileExistsExt('workspace.ini', Value)
    and not FileExistsExt('aquabase.exe', Value)
...
begin
  ...
  else if PageControl1.ActivePage.Tag = 2 then
  with ZConnection1 do
  begin
    HostName := EditHostMySQL.Text;
    Port := StrToInt(EditPortMySQL.Text);
  ...
  else if PageControl1.ActivePage.Tag = 3 then
  with ZConnection1 do
  begin
    HostName := EditHostMaria.Text;
    Port := StrToInt(EditPortMaria.Text);
  ...
  else if PageControl1.ActivePage.Tag = 4 then
  begin
    with ZConnection1 do
    begin
      HostName := EditHostPostgre.Text;
  ...
  else if PageControl1.ActivePage.Tag = 5 then
  begin
    with ZConnection1 do
    begin
      HostName := EditHostMSsql.Text;
...
procedure TNewWSpaceForm.ZQueryCountriesAfterOpen(DataSet: TDataSet);
var
  lang: String;
begin
...
procedure TNewWSpaceForm.ZQueryCountriesBeforeOpen(DataSet: TDataSet);
begin
  with ZQueryCountries do
  begin
    SQL.Clear;
