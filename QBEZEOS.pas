...
uses
  types, Dialogs, SysUtils, Classes, DB, ZDataset, ZConnection, QBuilder,
...
var
  MyDataExporter: TFPDataExporter;
  ExpFormats: TStandardExportFormats;
begin
  ExpFormats := TStandardExportFormats.Create(nil);
  ExpFormats.Formats := [sefCSV,sefFixedLength,sefSimpleXMl,sefXMLXSD,sefSimpleJSON,sefSQL,seRTF,sefDBF];
  ExpFormats.Active := True;
  MyDataExporter := TFPDataExporter.Create(nil);
  MyDataExporter.Dataset := FResultQuery;
  if FResultQuery.Active and (FResultQuery.RecordCount > 0) then
    MyDataExporter.Execute
  else
    ShowMessage('There are no query results to export! You have to generate and then run the query to obtain results first.');
  MyDataExporter.Free;
  ExpFormats.Free;
end;
