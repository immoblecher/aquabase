...
type
  ...
  { TSurfWatrRepSettingsForm }

  TSurfWatrRepSettingsForm = class(TReportSettingsForm)
    CheckBoxAMSL: TCheckBox;
...
var
  SurfWatrRepSettingsForm: TSurfWatrRepSettingsForm;

implementation

{$R *.lfm}

uses surfacewaterreport, VARINIT, Repprev;

{ TSurfWatrRepSettingsForm }

procedure TSurfWatrRepSettingsForm.CheckGroup1ItemClick(Sender: TObject;
  Index: integer);
var
  i: Byte;
...
begin
  ...
  CheckBoxAMSL.Enabled := CheckGroup1.Checked[0];
end;

procedure TSurfWatrRepSettingsForm.FormShow(Sender: TObject);
begin
  DateTimePicker2.DateTime := now;
end;

procedure TSurfWatrRepSettingsForm.OKButtonClick(Sender: TObject);
begin
  with TSurfaceWaterReportForm.Create(NIL) do
  begin
